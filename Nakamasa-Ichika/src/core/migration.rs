...
fn get_migrations() -> Vec<Migration> {
    vec![
        ...
        Migration {
            version: "1.0.4",
            description: "用户表新增人脸字段与异地登录记录（face_embedding/face_image/face_time/last_location/last_login_ip）、app.face_store_image 保留底图开关",
            migration_type: MigrationType::Both,
            sql: Some(SqlSource::Statements(&[
                r#"ALTER TABLE `{user}`
                    ADD COLUMN IF NOT EXISTS face_embedding BLOB DEFAULT NULL COMMENT '人脸特征向量(512维f32,2048字节)',
                    ADD COLUMN IF NOT EXISTS face_image VARCHAR(255) DEFAULT NULL COMMENT '人脸底图路径(开启保留底图时写入)',
                    ADD COLUMN IF NOT EXISTS face_time BIGINT(10) DEFAULT NULL COMMENT '人脸注册时间',
                    ADD COLUMN IF NOT EXISTS last_location VARCHAR(128) DEFAULT NULL COMMENT '最近登录地',
                    ADD COLUMN IF NOT EXISTS last_login_ip VARCHAR(45) DEFAULT NULL COMMENT '最近登录IP'"#,
            ])),
        },
        Migration {
            version: "1.0.5",
            description: "应用表新增 face_enable 开关（是否开启人脸识别安全验证）",
            migration_type: MigrationType::Both,
            sql: Some(SqlSource::Statements(&[
                r#"ALTER TABLE `{app}`
...
"#);
    ...
    // 老配置没有 app.face_store_image 开关时补上默认值（false=只存特征向量，不保留底图）
    content = ensure_face_store_image_key(&content);
...
/// 确保 `app.face_store_image` 键存在，缺失时以默认值 `false` 补写。
fn ensure_face_store_image_key(content: &str) -> String {
    if content.lines().any(|l| l.trim_start().starts_with("face_store_image:")) {
    ...
    for line in content.lines() {
        ...
        if !inserted {
            let indent = line.chars().take_while(|c| c.is_whitespace()).count();
            if indent == 0 && line.trim() == "app:" {
                out.push_str("    # 是否保留人脸底图（原始人脸图片）。false=仅存512维人脸特征向量（默认，更隐私）；true=同时存底图\n");
                out.push_str("    face_store_image: false\n");
                inserted = true;
            }
        }
    }
    if !inserted {
        out.push_str("app:\n    face_store_image: false\n");
    }
    out
}
