...
///
/// // 失效缓存
/// state.invalidate_user_cache(123, 12345);
/// ```
pub struct AppState {
    // ========================================================================
    // 数据存储层
    ...
    /// 用户基本信息缓存 ((appid, uid) -> UserInfoCache)
    ///
    /// 高频访问的用户基本信息，减少数据库查询。
    /// 容量：50,000，TTL：5分钟，策略：Hybrid
    /// key 含应用维度，避免多应用 uid 串读
    pub user_info_cache: Arc<ShardedCacheV2<(u64, u64), UserInfoCache>>,
...
pub struct CachedTokenData {
    ...
    /// 密码哈希
    pub password: String,
    /// 是否低权限 token（异地登录未完成身份验证时为 true）
    pub perm_low: bool,
    /// Token 过期时间 (Unix 时间戳)
    pub expires_at: i64,
}
...
impl AppState {
    ...
    /// ```rust,ignore
    /// // 用户修改密码后
    /// app_state.invalidate_user_cache(appid, user_id);
    /// ```
    #[inline]
    pub fn invalidate_user_cache(&self, appid: u64, uid: u64) {
        self.user_info_cache.remove(&(appid, uid));
        tracing::debug!("用户缓存已失效: appid={}, uid={}", appid, uid);
    }
    ...
    /// 同时失效用户信息缓存和相关的会话数据。
    #[inline]
    pub fn invalidate_user_all(&self, appid: u64, uid: u64) {
        self.invalidate_user_cache(appid, uid);
        // 可以添加更多相关缓存的失效逻辑
        tracing::info!("用户所有缓存已失效: appid={}, uid={}", appid, uid);
    }
