...
pub async fn get_list(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    if let Some(so) = list_req.so {
            ...
            && let Some(keyword_type) = so.keyword_type
        {
            ...
            } else if matches!(keyword_type.as_str(), "cdk" | "note" | "use_ip") {
                // 按字段搜索（动态列名走白名单，禁止拼接任意前端字段）
...
pub async fn add(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    for _ in 0..add_req.num {
        ...
        // 使用绑定参数执行，避免手工转义插值（H4: 杜绝 NO_BACKSLASH_ESCAPES 注入）
        let mut statement = sqlx::query(&query);
        for (ty, val) in &params {
            statement = match ty.as_str() {
                "u64" | "i64" => statement.bind(val.parse::<i64>().unwrap_or(0)),
                _ => statement.bind(val.as_str()),
            };
        }

        match statement.execute(&mut *tx).await {
            Ok(r) if r.rows_affected() > 0 => {
                success_count += 1;
            }
            Ok(_) => {
                tracing::error!("插入失败: rows_affected = 0");
                if let Err(rollback_err) = tx.rollback().await {
                    tracing::error!("事务回滚失败: {}", rollback_err);
                }
                res.render(Json(ApiResponse::<()>::error(
                    "创建失败: 卡密插入无影响行".to_string(),
                    201,
                )));
                return;
            }
            Err(e) => {
                tracing::error!("插入失败: {}", e);
                if let Err(rollback_err) = tx.rollback().await {
                    tracing::error!("事务回滚失败: {}", rollback_err);
                }
                res.render(Json(ApiResponse::<()>::error(
...
operation_log::log_admin(db, admin_id, "cdkUser_edit", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "cdkUser_del", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
operation_log::log_admin(db, admin_id, "cdkUser_editState", None, &ip, None);
                res.render(Json(ApiResponse::success_msg("编辑成功")));
            } else {
                res.render(Json(ApiResponse::<()>::error("编辑失败", 201)));
            }
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "cdkUser_delall", None, &ip, None);
                res.render(Json(ApiResponse::success_msg("删除成功")));
            } else {
                res.render(Json(ApiResponse::<()>::error("删除失败", 201)));
            }
...
pub async fn out_all(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    match result {
        Ok(rows) => {
            ...
            let _ = update_sql.execute(db).await;
