    ...
    match insert_result {
        Ok(result) => {
            if result.rows_affected() > 0 {
                ...
                operation_log::log_admin(db, admin_id, "agentGroup_add", None, &ip, Some(appid));
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                operation_log::log_admin(db, admin_id, "agentGroup_edit", None, &ip, Some(appid));
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                let appid = req.headers().get("appid").and_then(|h| h.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "agentGroup_del", None, &ip, appid);
