...
#[handler]
pub async fn clear_cache(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    // 清除缓存功能暂时返回成功
    ...
    if let Ok(app_state) = depot.get_typed::<Arc<AppState>>() {
        if let Some(db) = app_state.get_db() {
            operation_log::log_admin(db, admin_id, "system_clearCache", None, &ip, appid_opt);
        }
    }
...
pub async fn edit_set(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    match result {
        Ok(_) => {
            ...
            let ip = get_client_ip(req).to_string();
            let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
            operation_log::log_admin(db, admin_id, "set_edit", None, &ip, appid_opt);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                operation_log::log_admin(db, admin_id, "user_api_router_edit", None, &ip, Some(appid));
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                operation_log::log_admin(db, admin_id, "user_api_router_switch", None, &ip, Some(appid));
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                operation_log::log_admin(db, admin_id, "user_api_code_edit", None, &ip, Some(appid));
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                operation_log::log_admin(db, admin_id, "user_api_code_switch", None, &ip, Some(appid));
