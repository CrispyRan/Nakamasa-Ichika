...
pub async fn get_list(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let data_total = match count_query_builder.fetch_one(db).await {
    ...
    match insert_result {
        Ok(result) => {
            if result.rows_affected() > 0 {
                ...
                operation_log::log_admin(db, admin_id, "encryption_add", None, &ip, appid_hdr);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                ...
                let ip = get_client_ip(req).to_string();
                let appid_hdr = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "encryption_edit", None, &ip, appid_hdr);
                app_state.invalidate_app_runtime_cache(appid);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                ...
                let ip = get_client_ip(req).to_string();
                let appid_hdr = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "encryption_del", None, &ip, appid_hdr);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                ...
                let ip = get_client_ip(req).to_string();
                let appid_hdr = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "encryption_editSign", None, &ip, appid_hdr);
                app_state.invalidate_app_runtime_cache(0);
