...
pub async fn get_list(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    // 查询数据
    let query = format!(
        "SELECT id, email, phone, acctno, nickname, avatars, inviter_id, vip, fen, extend, open_wx, open_qq, reg_time, reg_ip, reg_sn, sn_list, sn_max, ban, ban_msg, appid, face_time, face_image, last_location, last_login_ip FROM u_user WHERE {} ORDER BY id DESC LIMIT ? OFFSET ?",
        where_clause
    );
    ...
    match result {
        Ok(rows) => {
            ...
            for row in rows.iter() {
                ...
                // 处理人脸底图地址（相对路径拼接完整URL）
                let face_image_raw: Option<String> = row.try_get("face_image").ok();
                let face_image = face_image_raw.and_then(|img| {
                    if img.is_empty() {
                        None
                    } else {
                        Some(format!("{}{}", app_url, img))
                    }
                });
                ...
                let user_item = UserItem {
                    ...
                    face_time: row.try_get("face_time").ok(),
                    face_image,
                    last_location: row.try_get("last_location").ok(),
                    last_login_ip: row.try_get("last_login_ip").ok(),
                };
                list.push(user_item);
            }
...
pub async fn add(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    // 密码加密: Argon2id（弃用无盐 MD5，避免 DB 泄露即离线秒破）
    let password_hash = match crate::core::password::hash_password(&add_req.password) {
        Ok(h) => h,
        Err(e) => {
            tracing::error!("用户密码哈希失败: {}", e);
            res.render(Json(ApiResponse::<()>::error("创建失败", 201)));
            return;
        }
    };
    let now = Utc::now().timestamp();
    ...
    match result {
        Ok(r) => {
            ...
            let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
            operation_log::log_admin(db, admin_id, "user_add", None, &ip, appid_opt);
...
pub async fn award(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    if success {
        ...
        let ip = get_client_ip(req).to_string();
        let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
        operation_log::log_admin(db, admin_id, "user_award", None, &ip, appid_opt);
...
pub async fn edit(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    {
        // Argon2id（弃用无盐 MD5）
        password_hash_opt = Some(match crate::core::password::hash_password(pwd) {
            Ok(h) => h,
            Err(e) => {
                tracing::error!("用户新密码哈希失败: {}", e);
                res.render(Json(ApiResponse::<()>::error("密码加密失败", 201)));
                return;
            }
        });
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                // 失效用户缓存
                if let Ok(appid_u64) = appid.parse::<u64>() {
                    app_state.invalidate_user_cache(appid_u64, edit_req.id);
                }

                let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
                let ip = get_client_ip(req).to_string();
                let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "user_edit", None, &ip, appid_opt);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                // 失效用户缓存
                if let Ok(appid_u64) = appid.parse::<u64>() {
                    app_state.invalidate_user_cache(appid_u64, del_req.id as u64);
                }
                // 统计面板缓存失效（用户数减少）
                if let Ok(appid_u64) = appid.parse::<u64>() {
                    app_state.invalidate_stats_cache(appid_u64);
...
pub async fn get(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let user_result = sqlx::query(
        "SELECT id, email, phone, acctno, nickname, avatars, password, inviter_id, vip, fen, extend, open_wx, open_qq, reg_time, reg_ip, reg_sn, sn_list, sn_max, ban, ban_msg, appid, face_time, face_image, last_location, last_login_ip FROM u_user WHERE id = ? AND appid = ?"
    )
    .bind(get_req.id)
    .bind(appid)
    .fetch_optional(db)
    ...
    let user_info = match user_result {
        Ok(Some(row)) => {
            ...
            UserInfo {
                ...
                face_time: row.try_get("face_time").ok(),
                face_image: row
                    .try_get::<Option<String>, _>("face_image")
                    .ok()
                    .flatten()
                    .and_then(|img| {
                        if img.is_empty() {
...
#[derive(Debug, Deserialize)]
struct DelFaceRequest {
    id: u64,
}

/// 删除用户人脸（清除特征向量与底图记录）
#[handler]
pub async fn del_face(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    let app_state = match depot.get_typed::<Arc<AppState>>() {
        Ok(s) => s,
        Err(_) => {
            res.render(Json(ApiResponse::<()>::error("服务器错误", 201)));
    ...
    let del_req = match req.parse_json::<DelFaceRequest>().await {
        Ok(data) => data,
        Err(_) => {
            res.render(Json(ApiResponse::<()>::error("参数解析失败", 201)));
            return;
    ...
    let result = sqlx::query(
        "UPDATE u_user SET face_embedding = NULL, face_image = NULL, face_time = NULL WHERE id = ? AND appid = ?",
    )
    .bind(del_req.id)
    .bind(appid)
    .execute(db)
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                res.render(Json(ApiResponse::success_msg("人脸已清除")));
            } else {
                res.render(Json(ApiResponse::<()>::error("用户不存在", 201)));
            }
        }
        Err(e) => {
            tracing::error!("删除人脸失败: {}", e);
            res.render(Json(ApiResponse::<()>::error("删除失败", 201)));
        }
    }
}

/// 管理员上传人脸覆盖特征（强制覆盖，不做旧脸相似度比对）
/// multipart 表单：`id`（必填）+ `file`（人脸图片）
#[handler]
pub async fn face_reg(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let form_data = match req.form_data().await {
        ...
        Err(e) => {
            ...
            res.render(Json(ApiResponse::<()>::error("解析表单数据失败", 201)));
            return;
        }
    };

    // 获取用户 id（multipart 字段）
    let id: u64 = match form_data.fields.get("id").and_then(|v| v.parse().ok()) {
        Some(id) => id,
        None => {
    ...
    let file = match form_data.files.get("file") {
        Some(f) => f,
        None => {
            res.render(Json(ApiResponse::<()>::error("缺少人脸图片", 201)));
            return;
        }
    };

    if file.size() > 5 * 1024 * 1024 {
        res.render(Json(ApiResponse::<()>::error("文件大小超过限制（最大5MB）", 201)));
        return;
    }

    let image_bytes = match std::fs::read(file.path()) {
        Ok(data) => data,
        Err(e) => {
            tracing::error!("读取人脸图片失败: {}", e);
            res.render(Json(ApiResponse::<()>::error("读取文件失败", 201)));
            return;
        }
    };

    let embedding = match crate::app::handlers::api::user::misc::face::extract_embedding(
        &image_bytes,
    ) {
        Ok(e) => e,
        Err(msg) => {
    ...
    let face_image: Option<String> = if app_state.config().app().face_store_image() {
        crate::app::handlers::api::user::misc::face::save_face_image(
    ...
    let enc =
        crate::app::handlers::api::user::misc::face::encode_embedding(&embedding);
    ...
    match update {
        Ok(r) if r.rows_affected() > 0 => {
            // 与 get/list 一致，返回完整可访问地址
            let app_url = app_state.config().app().host().to_string();
            let face_image_url = face_image
                .filter(|s| !s.is_empty())
                .map(|s| format!("{}{}", app_url, s));
            res.render(Json(ApiResponse::success(
                "人脸特征已覆盖",
                Some(serde_json::json!({ "faceImage": face_image_url })) as Option<serde_json::Value>,
            )));
        }
        _ => {
            res.render(Json(ApiResponse::<()>::error("用户不存在或更新失败", 201)));
        }
    }
}
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                ...
                // 记录日志
                let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "updateInfo", None, &ip, appid_opt);
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                ...
                let ip = get_client_ip(req).to_string();
                let appid_opt = req.headers().get("appid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
                operation_log::log_admin(db, admin_id, "modifyPassword", None, &ip, appid_opt);
