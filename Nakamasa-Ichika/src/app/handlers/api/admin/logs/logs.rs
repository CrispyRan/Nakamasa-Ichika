...
operation_log::log_admin(db, admin_id, "logs_del", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "logs_delall", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "logs_clean", None, &ip, None);
            res.render(Json(ApiResponse::success_msg(msg)));
        }
        Err(e) => {
