...
pub async fn get_list(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let mut sql_query =
        sqlx::query_as::<_, (u64, String, String, i64, i64, String, String, i64)>(&query);
    for param in &params {
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                // 商品变更，失效用户端商品列表缓存
                app_state.invalidate_goods_cache(appid as u64);
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "goods_add", None, &ip, Some(appid as u64));
...
pub async fn edit(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    .bind(edit_req.val)
    .bind(edit_req.money.round() as i64)
    ...
    match result {
        Ok(r) => {
            if r.rows_affected() > 0 {
                // 商品变更，失效用户端商品列表缓存
                app_state.invalidate_goods_cache(0);
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "goods_edit", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "goods_editState", None, &ip, None);
                res.render(Json(ApiResponse::success_msg("编辑成功")));
            } else {
                res.render(Json(ApiResponse::<()>::error("编辑失败", 201)));
            }
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "goods_del", None, &ip, None);
