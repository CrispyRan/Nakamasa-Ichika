...
pub async fn add(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    match result {
        Ok(r) => {
            ...
            operation_log::log_admin(db, admin_id, "functions_add", None, &ip, Some(appid));
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "functions_edit", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "functions_del", None, &ip, None);
...
let admin_id = *depot.get::<u64>("admin_id").unwrap_or(&0);
let ip = get_client_ip(req).to_string();
operation_log::log_admin(db, admin_id, "functions_editState", None, &ip, None);
                res.render(Json(ApiResponse::success_msg("编辑成功")));
            } else {
                res.render(Json(ApiResponse::<()>::error("编辑失败", 201)));
            }
