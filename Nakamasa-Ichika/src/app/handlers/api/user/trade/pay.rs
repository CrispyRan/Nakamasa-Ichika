...
pub async fn pay(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    // 订单号 = 秒级时间戳 + 毫秒(3位) + 6位随机；量级比「秒+5位随机」大 1000 倍，
    // 配合 u_order.order_no 唯一索引，基本杜绝高并发碰撞导致回调错配。
    let order_no = format!(
        "{}{:06}",
    ...
    if let Some(inv_uid) = inviter_id {
            ...
            && divide > 0.0
        {
            ...
            order_data.insert(
                "divide_money",
                serde_json::json!((money as f64 * divide / 100.0).round() as i64),
            );
        }
    }
    ...
    let divide_money_val = order_data.get("divide_money").and_then(|v| v.as_i64());
