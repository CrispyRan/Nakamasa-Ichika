...
async fn handle_user_fen_verify(
    ...
    if let Err(e) = insert_result {
        // 唯一键冲突（并发重复兑换同一 mark）→ 走「已兑换」提示而不是笼统报错
        if let Some(db_err) = e.as_database_error()
            && db_err.code().map(|c| c.to_string()).as_deref() == Some("1062")
        {
    ...
    let update_result = if fen_event.vip > 0 {
        ...
        sqlx::query(
            "UPDATE u_user SET fen = fen - ?, vip = ? WHERE id = ? AND appid = ? AND fen >= ?",
        )
        .bind(fen_event.fen)
        .bind(new_vip)
        .bind(uid)
        ...
        sqlx::query("UPDATE u_user SET fen = fen - ? WHERE id = ? AND appid = ? AND fen >= ?")
            .bind(fen_event.fen)
            .bind(uid)
            .bind(appid)
            .bind(fen_event.fen)
    ...
    match update_result {
        Ok(r) if r.rows_affected() > 0 => {
            ...
            // 记录日志
            operation_log::log_user(db, "user", uid as u64, "fen", Some(serde_json::json!({"name": &fen_event.name})), ip, Some(appid));

            render_success_with_msg(res, "验证成功", app_key);
        }
        Ok(_) => {
            // 积分不足或并发被扣光，回滚订单
            tracing::warn!("积分扣减失败（余额不足或并发竞态）: uid={}, fen={}", uid, fen_event.fen);
            if let Err(re) = tx.rollback().await {
                tracing::error!("事务回滚失败: {}", re);
            }
            render_error(res, "积分余额不足", 201, app_key);
...
async fn handle_kami_fen_verify(
    ...
    // 更新卡密积分（带余额守卫，防并发透支）
    let update_result =
        sqlx::query("UPDATE u_cdk_kami SET val = val - ? WHERE id = ? AND appid = ? AND val >= ?")
            .bind(fen_event.fen)
            .bind(uid)
            .bind(appid)
            .bind(fen_event.fen)
    ...
    match update_result {
        Ok(r) if r.rows_affected() > 0 => {
            ...
            // 记录日志
            operation_log::log_user(db, "kami", uid as u64, "fen", Some(serde_json::json!({"name": &fen_event.name})), ip, Some(appid));

            render_success_with_msg(res, "验证成功", app_key);
        }
        Ok(_) => {
            // 积分不足或并发被扣光，回滚订单
            tracing::warn!("卡密积分扣减失败（余额不足或并发竞态）: uid={}, fen={}", uid, fen_event.fen);
            if let Err(re) = tx.rollback().await {
                tracing::error!("事务回滚失败: {}", re);
            }
            render_error(res, "积分余额不足", 201, app_key);
