...
pub async fn cloud_function(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    match result {
        Ok(json_result) => {
            ...
            // 如果消耗积分，更新用户积分（带余额守卫，防并发透支）
            if fen > 0 && app_type == "user" {
                let deduct = sqlx::query(
                    "UPDATE u_user SET fen = fen - ? WHERE id = ? AND appid = ? AND fen >= ?",
                )
                .bind(fen)
                .bind(user_info.uid)
                .bind(appid)
                ...
                match deduct {
                    Ok(r) if r.rows_affected() > 0 => {
                        app_state.invalidate_user_cache(appid, user_info.uid);
                    }
                    Ok(_) => {
                        tracing::warn!(
                            "云函数积分扣减失败（余额不足或并发竞态）: uid={}, fen={}",
                            user_info.uid,
                            fen
                        );
                    }
                ...
                operation_log::log_user(
                    ...
                    user_info.uid,
                    "cloudFunction",
                    Some(serde_json::json!({"name": cf_req.name, "fen": fen})),
                    &client_ip,
                    Some(appid),
                );
            }
