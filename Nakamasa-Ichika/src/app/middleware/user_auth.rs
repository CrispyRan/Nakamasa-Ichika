...
pub struct TokenData {
    ...
    pub p: String, // 密码hash
    /// 权限等级：`"low"` 表示低权限 token（异地登录未完成身份验证），
    /// 缺失或非 `"low"` 视为正常权限。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub lv: Option<String>,
}

/// 低权限 token 可访问的接口白名单（仅能走"身份验证"流程）：
/// - 人脸识别校验
/// - 验证码验证
const LOW_PRIV_ALLOWED_PATHS: &[&str] = &["/faceVerify", "/codeVerify"];
...
impl Handler for UserAuth {
    async fn handle(
        ...
        // 低权限 token（异地登录未完成身份验证）：仅允许走人脸识别/验证码验证接口
        if depot.get::<bool>("user_perm_low").map(|b| *b).unwrap_or(false) {
            let path = req.uri().path().to_string();
            let allowed = LOW_PRIV_ALLOWED_PATHS.iter().any(|p| path.ends_with(p));
            if !allowed {
                res.render(Json(ApiResponse::<()>::error_static(
                    "异地登录，请先完成身份验证",
                    134,
                )));
                ctrl.skip_rest();
    ...
    async fn token_check_internal(
        ...
        if let Some(cached) = app_state.token_cache.get(&token_cache_key) {
            ...
            if cached.is_valid(current_time) && cached.appid == app_id {
                ...
                depot.insert("user_perm_low", cached.perm_low);
        ...
        let perm_low = token_data.lv.as_deref() == Some("low");
        ...
        depot.insert("user_info", user_info);
        depot.insert("token", token_str);
        depot.insert("user_perm_low", perm_low);
...
/// 升级 token 权限：异地登录完成身份验证后，原地去掉低权限标记（lv=low），
/// 并失效 L1 缓存使下次请求重新从 Redis 读取。
///
/// 安全性（防 token 伪造）：
/// - 只能在已通过身份验证的 handler 内触发（异地登录白名单接口）。
/// - 通过 Redis Lua 脚本**原子**升级：仅当 token 存在、属主 `uid` 匹配、且仍为 low
///   权限时才去掉 `lv`；即使并发发生登出删除，也不存在"删除后被复活"的 TOCTOU 窗口。
/// - 不签发新 token、不改动 token 串，仅修改 Redis 中同一记录的权限标记。
pub async fn upgrade_token_privilege(
    app_state: &Arc<AppState>,
    token_pre: &str,
    token: &str,
    uid: u64,
    ...
    let redis_pool = match app_state.redis_pool.as_ref() {
        Some(pool) => pool,
        None => return Err("Redis未初始化".to_string()),
    };
    let token_key = format!("{}{}", token_pre, token);
    let uid_str = uid.to_string();
    let exp_str = token_exp.to_string();

    // 原子升级脚本：返回 "1"=升级成功，其他=拒绝
    let script = r#"
local val = redis.call('GET', KEYS[1])
if not val then return '0' end
local ok, data = pcall(cjson.decode, val)
if not ok or type(data) ~= 'table' then return '0' end
if data.lv ~= 'low' then return '0' end
if tostring(data.uid) ~= ARGV[1] then return '0' end
data.lv = nil
local new = cjson.encode(data)
redis.call('SETEX', KEYS[1], tonumber(ARGV[2]), new)
return '1'
"#;

    let upgraded = match app_state
        .redis_util
        .eval(redis_pool, script, &[&token_key], &[&uid_str, &exp_str])
        .await
    {
        Ok(Some(s)) => s == "1",
        Ok(None) => false,
        Err(e) => {
            tracing::warn!("升级 token 权限失败: {}", e);
            false
        }
    };

    // 升级成功才失效 L1 token 缓存，强制下次走 Redis 读取最新权限
    if upgraded {
        app_state
            .token_cache
            .remove(&AppState::token_cache_key(token));
    } else {
        // 记录安全日志便于审计（token不存在 / 非low / uid不匹配 / 已被升级）
        tracing::warn!(
            "token 权限升级被拒绝(uid={}): 仅当 token 为 low 且属主匹配时才允许升级",
            uid
        );
    }
...
async fn fetch_user_info_optimized(
    ...
    // ========== 先尝试从缓存获取 ==========
    if user_type == "user"
        && let Some(cached) = app_state.user_info_cache.get(&(appid, uid))
    {
        // 缓存命中，检查 appid 匹配
        if cached.uid == uid {
        ...
        && user_type == "user"
    {
        ...
        app_state.user_info_cache.set((appid, uid), cached);
        tracing::trace!("用户信息已缓存: appid={}, uid={}", appid, uid);
    }
